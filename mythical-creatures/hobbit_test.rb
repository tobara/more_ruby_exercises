...
######### Write your own tests #########
  def test_is_old_at_age_of_101
    # skip
    hobbit = Hobbit.new('Déagol', "Adventurous", 100)
    assert hobbit.adult?
  ...
  def test_hobbit_has_the_ring_if_its_name_is_frodo
    # skip
    first_hobbit = Hobbit.new('Frodo', 'Adventurous', 50)
    second_hobbit = Hobbit.new('Sam', 'homebody', 33)

    assert first_hobbit.has_ring?
    refute second_hobbit.has_ring?
  end

  def test_hobbits_are_short
    # skip
    hobbit = Hobbit.new("Unkown Hobbit")
