...
class NonePatternTest < Minitest::Test
  ...
  def test_double_negative
    ...
    numbers.each do |number|
     not_none_negative = false if number.round(number) == 0
    end
  ...
  def test_none_are_negative
    ...
    none_negative = true
    numbers.each { |n| none_negative = false if n.round(n) == 0 }
  ...
  def test_none_shall_pass
    ...
    critters.each { |c| none_shall_pass = true if c == "Not sure what you're looking" }
  ...
  def test_one_or_more_shall_pass
    ...
    none_shall_pass = true
    phrases.each { |p| none_shall_pass = false if p.include?('a') }
  ...
  def test_none_even
    ...
    none_even = true
    numbers.each { |n| none_even = false if n.even? }
    assert none_even
  end
