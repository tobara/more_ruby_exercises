...
class ReduceTest < Minitest::Test
  ...
  def test_multiply_list_of_numbers
    # skip
    numbers = [2, 3, 5, 7]
    result = numbers.reduce(1) do |product, factor|
      product * factor
    end
  ...
  def test_capitalize_keywords_in_phrase_one_fish_two_fish_red_fish_blue_fish
    # skip
    keywords = ["fish", "blue"]
    result = keywords.reduce('one fish two fish red fish blue fish') do |v, k|
        v.gsub!(k, k.upcase)
    end
  ...
  def test_divide_560_by_a_bunch_of_numbers
    # skip
    numbers = [2, 2, 2, 5, 7]
    result = numbers.reduce(560) { |q,n| q /= n }
  ...
  def test_subtract_smallest_values_from_100
    ...
    result = elements.reduce(100) { |d,e| d -= e.min }
  ...
  def test_add_all_the_second_values_together
    ...
    result = elements.reduce(0) { |s,e| s += e.last }
